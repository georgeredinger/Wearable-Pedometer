...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    CGFloat leftBoundary = 22.0f + ((indicatorSpace + indicatorDiameter) * currentPage) - indicatorSpace/2;
    CGFloat rightBoundary = leftBoundary + indicatorDiameter + indicatorSpace;
    CGFloat hitArea = rightBoundary - leftBoundary;
    
	// check whether the touch is in the right or left hand-side of the control
    double step = 0;
	if (touchLocation.x < leftBoundary) {
        step = (leftBoundary - touchLocation.x)/hitArea;
		self.currentPage = MAX(self.currentPage - (int)ceil(step), 0) ;
	} else  if (touchLocation.x > rightBoundary) {
        step = (touchLocation.x - rightBoundary)/hitArea;
		self.currentPage = MIN(self.currentPage + (int)ceil(step), numberOfPages - 1) ;
    }
