...
/** Macro to set the Security Status for current link. */
#define SET_CONN_SEC_UPDATE_STATUS()                                                              \
        do                                                                                        \
        {                                                                                         \
             m_sec_con_status = true;                                                             \
        }while (0)

/** Macro to get the Security Status for current link. */
#define GET_CONN_SEC_UPDATE_RCVD()  (m_sec_con_status)

/** Macro to reset the Security Status for current link. */
#define RESET_CONN_SEC_UPDATE_STATUS()                                                            \
        do                                                                                        \
        {                                                                                         \
            m_sec_con_status = false;                                                             \
        } while (0)

/** @} */

/**@brief The macros to verify Module Initialization Status
 * @{
 */

/**
 * Macro returning NRF_INVALID_STATE in case a module API is called without initializing the module
 */
#define VERIFY_MODULE_INITIALIZED()                                                               \
        do                                                                                        \
        {                                                                                         \
            if(false == m_is_bondmngr_initialized)                                                \
            {                                                                                     \
...
static uint8_t             m_sec_con_status;
...
static void update_whitelist(void)
{
    ...
    for (i = 0, m_addr_count = 0, m_irk_count = 0; i < m_masters_in_db_count; i++)
    {
        ...
        if ((p_bond->auth_status.central_kex.irk)
            &&(p_bond->master_addr.addr_type != BLE_GAP_ADDR_TYPE_PUBLIC)
            &&(p_bond->master_addr.addr_type != BLE_GAP_ADDR_TYPE_RANDOM_STATIC)
            &&(p_bond->master_addr.addr_type != BLE_GAP_ADDR_TYPE_RANDOM_PRIVATE_NON_RESOLVABLE)
            )
        {
...
/**@brief Function for checking if two variables of the type ble_gap_sec_levels_t are equal.
 *
...
/**@brief Function for checking if two variables of the type ble_gap_sec_keys_t are equal.
 *
...
/**@brief Function for checking if two variables of the type ble_gap_enc_info_t are equal.
 *
...
/**@brief Function for checking if two variables of the type ble_gap_irk_t are equal.
 *
...
/**@brief Function for checking if two variables of the type ble_gap_addr_t are equal.
 *
...
/**@brief Function for checking if two variables of the type ble_gap_evt_auth_status_t are equal.
 *
...
static void on_sys_attr_missing(ble_evt_t * p_ble_evt)
{
    ...
    if ((m_master.bond.master_handle == INVALID_MASTER_HANDLE) || (false == GET_CONN_SEC_UPDATE_RCVD()))
    {
        err_code = sd_ble_gatts_sys_attr_set(m_conn_handle, NULL, 0);
    }
    else
...
static void on_auth_status(ble_gap_evt_auth_status_t * p_auth_status)
{
    ...
    if (m_master.bond.master_handle == INVALID_MASTER_HANDLE)
    {
        ...
        {
            case NRF_SUCCESS:
                // Receiving BLE_GAP_EVT_AUTH_STATUS on an existing master implies
                // previously bonded master has lost bonding information is rebonding.
                // Therefore, all the system attributes wrt this master need not be
                // remembered. Remove any previously stored bonding information.
                err_code = ble_bondmngr_bonded_master_delete (m_master.bond.master_handle);
                break;

            case NRF_ERROR_NOT_FOUND:
                // Master not found, new master!
                break;

            default:
                // Currently no other return code returned by master_find_in_db.
                break;
        }

        // Ensure the BLE_GAP_EVT_CONN_SEC_UPDATE is received
        // already as it is a another indication, this is bonding
        // request and not a pairing.
        //
        if (true == GET_CONN_SEC_UPDATE_RCVD())
        {
...
uint32_t ble_bondmngr_is_link_encrypted (bool * status)
{
    ...
    /* Note: Master handle is unused, however, is retained here in order to ensure
     * no change interface with future topology cha
     */
    (*status) = GET_CONN_SEC_UPDATE_RCVD();
    return NRF_SUCCESS;
}
