...
#define RTC_COMPARE_OFFSET_MIN  3                                           /**< Minimum offset between the current RTC counter value and the Capture Compare register. */
#define MAX_RTC_TASKS_DELAY     47                                          /**< Maximum delay until an RTC task is executed. */

/**@brief Timer accodation state type. */
typedef enum
{
 ...
 *          RTOS. The porpose of the queues is to be able to have a completely lockless timer
 *          implementation.
 */
typedef struct
{
...
static uint8_t                       m_ticks_elapsed_first;                     /**< Timer internal elapsed ticks queue read index. */
static uint8_t                       m_ticks_elapsed_last;                      /**< Timer internal elapsed ticks queue write index. */
 ...
 *
 * @param[in] prescaler   Value of the RTC1 prescale register. Set to 0 for no prescaling.
 */
...
{
    return ((ticks_now - ticks_old) & MAX_RTC_CNT);
}


/**@brief Function for setting the RTC1 Capture Compare register 0, and enabling the corresponding event.
 *
...
/**@brief Function for executing an application timeout handler, either by calling it directly, or by passing an
 *        event to the @ref app_scheduler.
 *
...
static bool elapsed_ticks_acquire(uint32_t * p_ticks_elapsed)
{
    ...
    if (m_ticks_elapsed_first != m_ticks_elapsed_last)
    {
        *p_ticks_elapsed = m_ticks_elapsed[m_ticks_elapsed_first];

        // Dequeue elapsed value 
        m_ticks_elapsed_first++;
        if (m_ticks_elapsed_first == CONTEXT_QUEUE_SIZE_MAX)
        {
            m_ticks_elapsed_first = 0;
        }

        m_ticks_latest += *p_ticks_elapsed;
        m_ticks_latest &= MAX_RTC_CNT;
                                   ...
                                   app_timer_id_t * p_restart_list_head)
{
    ...
    while (m_timer_id_head != TIMER_NULL)
    {
        ...
        {
            p_timer->ticks_at_start       = (ticks_previous + ticks_expired) & MAX_RTC_CNT;
...
static bool list_insertions_handler(app_timer_id_t restart_list_head)
{
    ...
    while (user_id--)
    {
        ...
        while ((restart_list_head != TIMER_NULL) || (p_user->first != p_user->last))
        {
            ...
            if (((p_timer->ticks_at_start - m_ticks_latest) & MAX_RTC_CNT) < (MAX_RTC_CNT / 2))
            {
 ...
 */
static timer_user_op_t * user_op_alloc(timer_user_t * p_user)
{
    app_timer_id_t    last;
